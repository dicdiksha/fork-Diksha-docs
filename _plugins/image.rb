  ...
  class ImageTag < Liquid::Tag
    ...
    def render(context)
      ...
      if zoom == 'zoom'
		...
		<span class="zoomclose" id="nopdf2">X</span>
		<figcaption id="nopdf3"><strong>Note:</strong> Click on the image. It expands in a pop-up window. Place your cursor on the expanded image and move or scroll your mouse to zoom-in further.<br /> To return to the normal view, click on the image again, or click anywhere outside the pop-up window, or press the <strong>Esc</strong> key.</figcaption>
		<img '+src.gsub('pages/usertaskflow/', '')+' '+@alt+' class="plugin '+position+' '+zoom+'" '+'width="'+image_width+'"/>
		</figure>'
	  else
      '<a target="_blank" '+src.gsub('src', 'href')+'>
        <img '+src.gsub('pages/features-documentation/', '')+' '+@alt+' class="plugin '+position+' '+zoom+'" '+'width="'+image_width+'"/>
      </a>'
      end
    end
  end
